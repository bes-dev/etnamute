...
Add features, remove features, optimize UX, fix bugs — keeping code clean and artifacts in sync.
...
### Step 5: Verify (ALL levels mandatory — see `pipeline/qa.md`)
...
**Level 2 — Tests:**
3. `npx jest --passWithNoTests` — all tests pass (including new ones)

**Level 3 — Runtime:**
4. Start app on simulator (`npx expo start --ios`), capture logs
5. Check for runtime errors (ERROR, TypeError, "is not a function", "is undefined")
6. If errors → fetch docs via mcpdoc, fix, re-run. Max 3 iterations.

ALL three levels must pass before reporting to user.
